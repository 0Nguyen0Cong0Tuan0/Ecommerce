...
const TableCategory = ({ categoryTableTitle, deleteCat, handleChangePage }) => {
    ...
    return (
        <div className="overflow-x-auto mt-5 rounded-xl">
            <table className="table-auto min-w-full text-left text-sm text-gray-300">
                <thead className="bg-gray-700 text-xs text-white">
                    <tr className="text-lg tracking-widest">
                        {categoryTableTitle.map((title, idx) => (
                ...
                <tbody>
                    {
                        context.catData?.categoryList?.length !== 0 && context.catData?.categoryList?.map((item, index) => (
                            <tr key={item._id} className="border-b bg-gray-800">
                                ...
                                <td className="py-2 px-4">
                                    {
                                        item.images.length > 0 && <img src={item.images[0].url} alt={item.name} className="w-16 h-16 object-cover rounded-md" />
                                    }
                                </td>
                                <td className="py-2 px-4">
                                ...
                                <td className="py-2 px-4">
                                    <div className='flex space-x-2'>
                                        ...
                                        <button className={`${actionBtnClass} text-white py-2 px-4 rounded`} onClick={() => deleteCat(item._id)}>
            ...
            {context.catData?.totalPages > 1 && (
                <div className="flex justify-center mt-4">
                    <Pagination
                        count={context.catData?.totalPages}
                        color='primary'
